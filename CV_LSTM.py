...
class BasicConvLSTMCell(object):
  ...
  def __init__(self, shape, filter_size, num_features, time_size,forget_bias=1.0, input_size=None,
...
#LSTM的最后输出结果
  ...
  def Full_connect(self, X, state):
      ...
      shapes=state_result.get_shape().as_list()
      print(shapes)
      nodes=shapes[1]*shapes[2]*shapes[3]
      reshaped=tf.reshape(state_result,[shapes[0],nodes])
      #第一个全连接层
...
#第二个全连接层
      with tf.variable_scope('F_Layer_two',reuse=tf.AUTO_REUSE):
          weight_three=tf.get_variable("weight",
                                   [self.ALL_SIZE_ONE,64],
                                   initializer=tf.truncated_normal_initializer(stddev=0.1))
          bias_three=tf.get_variable("bias",[64],
                                   initializer=tf.constant_initializer(0.1))
          layer2=tf.nn.relu(tf.matmul(layer1,weight_three)+bias_three)
      with tf.variable_scope('F_Layer_three',reuse=tf.AUTO_REUSE):
          weight_four=tf.get_variable("weight",
                                   [64,self.ALL_SIZE_TWO],
                                   initializer=tf.truncated_normal_initializer(stddev=0.1))
          bias_four=tf.get_variable("bias",[self.ALL_SIZE_TWO],
                                   initializer=tf.constant_initializer(0.1))
          layer3=tf.nn.relu(tf.matmul(layer2,weight_four)+bias_four)
#        如果keep_out不等于None，则使用dropout函数，任何一个给定单元的留存率
#        if(KeepProb!=1):
